...
# Set the OpenAI API key (must start with sk- or sk-proj-)
...
def _is_valid_openai_key(key: str) -> bool:
    k = (key or "").strip()
    return len(k) > 20 and (k.startswith("sk-") or k.startswith("sk-proj-"))

if openai_api_key:
    openai_api_key = openai_api_key.strip()
    if not _is_valid_openai_key(openai_api_key):
        st.error(
            "That doesn't look like a valid OpenAI API key. Keys start with **sk-** or **sk-proj-** "
            "and are long. Get one at https://platform.openai.com/account/api-keys"
        )
        st.stop()
    ...
    class CustomerSupportAIAgent:
        def __init__(self, api_key: str):
            ...
            config = {
                ...
                "llm": {"provider": "openai", "config": {"api_key": self._api_key}},
                "embedder": {"provider": "openai", "config": {"api_key": self._api_key}},
            }
            try:
        ...
        def generate_synthetic_data(self, user_id: str) -> dict | None:
            try:
                ...
                if not content:
                    st.error("No content in model response.")
                    return None
    ...
    # Initialize the CustomerSupportAIAgent with the key from the input
    support_agent = CustomerSupportAIAgent(api_key=openai_api_key)
